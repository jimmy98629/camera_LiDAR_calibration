...
def extract_3d_points(path):
    ...
    max_range = (np.array([lidar_data[:, 0].max() - lidar_data[:, 0].min(), 
        ...
        lidar_data[:, 2].max() - lidar_data[:, 2].min()]).max() / 2.0)
...
def calibrate(points_2d, points_3d):
    ...
    success, rotation_matrix, translation_matrix, inlier = cv2.solvePnPRansac(points_2d,
    points_3d, intrinsic_cam_matrix, dist_coeff, flags=cv2.SOLVEPNP_ITERATIVE)
